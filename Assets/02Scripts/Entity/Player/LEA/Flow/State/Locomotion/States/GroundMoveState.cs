...
public class GroundMoveState : BaseState
{
    ...
    {
        // ==================== Input
        bool isSprintPress = _Core.Input.IsSprint;
        bool isJumpPress = _Core.Input.IsJump;
        bool isDashPress = _Core.Input.IsDash;
        ...
        // ==================== Rot
        Vector2 mouseInput = _Core.Input.MouseInputPos;
        ...
        {
            _Core.StateMachineFlow.ChangeLocoState(ELocomotionType.Fall);
        }
        else if (isJumpPress && _Core.LocoRule.CanJump(_Core.Context))
        {
            _Core.StateMachineFlow.ChangeLocoState(ELocomotionType.Jump);
        }
        else if (isDashPress && _Core.LocoRule.CanDash(_Core.Context))
        {
            _Core.StateMachineFlow.ChangeLocoState(ELocomotionType.Dash);
        }
    }
