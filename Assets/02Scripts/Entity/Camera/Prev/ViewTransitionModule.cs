...
public class ViewTransitionModule : MonoBehaviour
{
    ...
    public bool ViewTransition(ECameraViewType p_prevView,CameraViewDataSO p_nextViewInfo)
    {
        RigFollow(true);
        ...
        if (p_nextViewInfo.ViewType == ECameraViewType.Quarter || p_prevView == ECameraViewType.Quarter)
        {
            _currentMouseX = 0;
            _currentMouseY = 0;
        ...
        else if(p_prevView != ECameraViewType.Quarter)
        {
    ...
    // ==================== View 전환 시 사용
    private bool TrasitionPivotPosition(CameraViewDataSO p_viewInfo)
    {
        Vector3 targetPivot = 
    ...
    // QuarterView만 사용
    private bool TrasitionPivotRotation(CameraViewDataSO p_viewInfo)
    {
    ...
    private bool TrasitionShoulderPosition(CameraViewDataSO p_viewInfo)
    {
    ...
    private bool TrasitionFOV(CameraViewDataSO p_viewInfo)
    {
    ...
    public bool TrasitionZoom(CameraViewDataSO p_viewInfo)
    {
        // 음수로 전환
        var targetZoomPos = 
    ...
    // ==================== RunTime
    public void RunTimeTPSView(Vector2 p_inputLookDir, float p_scrollInput, CameraViewDataSO p_viewData)
    {
        RigFollow(false, _rigFollowSpeed[(int)ECameraViewType.ThirdPerson]);
    ...
    {
        RigFollow(false, _rigFollowSpeed[(int)ECameraViewType.Aim]);
        RunTimePivotRotation(false, p_inputLookDir);
    }

    public void RunTimeQuarterView()
    {
        RigFollow(false, _rigFollowSpeed[(int)ECameraViewType.Quarter]);
    }
