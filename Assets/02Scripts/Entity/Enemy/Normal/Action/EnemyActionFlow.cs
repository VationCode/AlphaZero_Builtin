...
namespace Alpha.Enemy
{
    ...
    public sealed class EnemyActionFlow : MonoBehaviour, IHitReactionReceiver
    {
        ...
        // Boss Cinematic 등 외부 흐름이 Enemy의 AI·이동·공격을 중첩 차단한다.
        ...
        // 피해와 직접 호출이 동일한 면역·상태 전이 경로를 사용한다.
            ...
            in AttackImpactInfo p_impact, Transform p_attacker, Vector3 p_direction)
        {
            if (!isActiveAndEnabled || _isDead || IsExternallyBlocked || _core == null ||
                p_attacker == null || _core.HealthModule == null || _core.HealthModule.IsDead ||
                _core.HealthModule.CurrentHealth <= 0f)
                return false;
            ...
            bool moved = KnockbackSystem.TryApply(
