...
public abstract class Weapon : MonoBehaviour
{
    ...
    // 공통 검증
    public bool TryAttack(in WeaponAttackContext p_context)
    {
        if (!IsInitialized || !p_context.IsValid)
            return false;

        return TryExecuteAttack(p_context);
    }

    // 실제 공격 방식은 구체 무기가 구현한다.
    protected abstract bool TryExecuteAttack(in WeaponAttackContext p_context);
}
