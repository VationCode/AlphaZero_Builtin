...
{
    // 공용 DetectionAreaSystem 결과를 Scene Gizmo로 확인하는 편집기 테스트 View다.
    public class DetectionAreaView : MonoBehaviour
    {
        [Header("Origin")]
        [SerializeField]
        ...
        private Transform OwnerRoot => _attackerRoot != null
            ? _attackerRoot
            : transform;
        ...
        [ContextMenu("Test Detection Area")]
        ...
        private void ExecuteQuery(bool p_logResult)
        {
            ...
            _hitCount = DetectionAreaSystem.Query(
                request,
                _overlapBuffer,
                _hitBuffer);
        ...
        private DetectionAreaRequest CreateRequest()
        {
        ...
        {
            Debug.Log(
                $"Detection Area Test: {_settings.Shape}, " +
                $"Hit Count: {_hitCount}",
                this);
